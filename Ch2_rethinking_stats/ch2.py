...
def _(alt, binom, np, pl):
    def grid_approx_ch2(
        p_grid_size: int = 50, prior: str = "uniform", success: int = 6, tosses: int = 9
    ):
        # define a grid. Possible values for p
        p_grid = np.linspace(0, 1, p_grid_size)
        # define a prior. Uniform in this case, so all values are equally likely
        priors = {
            "uniform": np.ones_like(p_grid),
            # Alternatively, we can use a non-uniform prior. Step-function
            "step": np.where(p_grid < 0.5, 0, 1),
            # Or we can use an exponential prior
        ...
        df = pl.DataFrame(
            {"probability_of_water": p_grid, "posterior_probability": posterior}
        )
        ...
        chart = (
            ...
            .encode(
                x=alt.X("probability_of_water", title="probability of water"),
                y=alt.Y("posterior_probability", title="posterior probability"),
            )
...
def _(grid_approx_ch2, mo):
    post_df, globe_tosses_chart = grid_approx_ch2(100)
...
def _(grid_approx_ch2, mo):
    # W, W, W
    ex_m1_1_df, chart_ex_m1_1 = grid_approx_ch2(p_grid_size=100, success=3, tosses=3)

    mo.ui.altair_chart(chart_ex_m1_1)
    return
